...
@section('title', '创建新文章  - Biecheng 别城')
			...
			<div class="body-mac body-mac-u">
				<form action="#" method="POST" onsubmit="return false;">
					...
					<div class="mac-input-wrap mac-input-wrap-q">
						<select class="select-mac">
							<option data-i="0">原创</option>
							<option data-i="1">转载</option>
						</select>
						<input style="width: 80%;" name="title" id="title" value="" class="mac-input" placeholder="标题"/>
					</div>
					...
					<div style="margin: 10px 0 3px 0;">
						<span class="editormd-span">
							请选择标签
							<input class="mac-input editormd-q-tag-input" placeholder="标签，如：php,java（用逗号,分隔）"/>
						</span>							
					</div>					
					...
					<div class="mac-input-wrap editormd-border mac-input-wrap-q" style="border: 2px solid #d6d6d6; border-radius: 15px;">
						<button class="mac-button" style="border-radius: 15px;" onclick="postQuestion()">发布文章</button>
					</div>																		
				</form>
			</div>
		</div>
...
<script>
	...
	{	
	    ...
	    var original = $('.select-mac option:selected').attr('data-i');
		...
		$.ajax({
			...
			success: function(res){
				if( res.success )
				{
					window.location.href = '/a/' + res.success;
				}
			},
			error: function(res){
				window.location.href = '/login';
